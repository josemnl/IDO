function [RFS_errors, RFS_falsePositiveErrors, RFS_falseNegativeErrors] = Scenario2_RFS(lidar_range, width, height, resolution, FreeSpaceDiscountFactor, images)
    ...
    for i = 1:sim_horizon
        ...
        if i == 35
            ...
            saveas(gcf,strcat('../figures/Experiment2.', images(1), '.svg'))
            hold off
            % Figure 4
            grid.show('local')
            hold on
            ...
            ylabel('')
            set(gca,'XTick',[0,10,20,30,40], 'YTick', [0,10,20,30,40])
            saveas(gcf,strcat('../figures/Experiment2.', images(4), '.svg'))
            hold off
        end
        if i == 40
            % Figure 2
            ...
            ylabel('')
            set(gca,'XTick',[0,10,20,30,40], 'YTick', [0,10,20,30,40])
            saveas(gcf,strcat('../figures/Experiment2.', images(2), '.svg'))
            hold off
            % Figure 5
            grid.show('local')
            hold on
            ...
            ylabel('')
            set(gca,'XTick',[0,10,20,30,40], 'YTick', [0,10,20,30,40])
            saveas(gcf,strcat('../figures/Experiment2.', images(5), '.svg'))
            hold off
        end
        if i == 45
            % Figure 3
            ...
            ylabel('')
            set(gca,'XTick',[0,10,20,30,40], 'YTick', [0,10,20,30,40])
            saveas(gcf,strcat('../figures/Experiment2.', images(3), '.svg'))
            hold off
            % Figure 6
            grid.show('local')
            hold on
            ...
            ylabel('')
            set(gca,'XTick',[0,10,20,30,40], 'YTick', [0,10,20,30,40])
            saveas(gcf,strcat('../figures/Experiment2.', images(6), '.svg'))
            hold off
        end
    end
    
    %% Save the video
    VW = VideoWriter(strcat('../videos/Scenario2_RFS_',string(FreeSpaceDiscountFactor),'.avi'));
    VW.FrameRate = 10;
    VW.open();
    for Frame = Frames
        VW.writeVideo(Frame)
